...
export class DatabaseMonitoringService {
  ...
  private schemaMetrics: Map<string, DatabaseSchemaMetrics> = new Map();
  ...
  private setupQueryLogging() {
    ...
    try {
      ...
      logger.warn('Query logging setup failed:', error);
    }
  }

  private extractSchemaFromQuery(query: string): string {
  ...
  private getQueryType(query: string): QueryMetrics['type'] {
  ...
  private extractRowsAffected(query: string): number | undefined {
  ...
  stopMonitoring() {
    ...
    logger.info('Stopped database monitoring service');
  }
  ...
  private async updateConnectionPoolMetrics() {
    try {
      ...
      try {
        ...
        logger.warn('Could not get detailed connection pool stats, using defaults');
  ...
  private async updateSchemaMetrics() {
    try {
      ...
      for (const schemaName of schemas) {
        try {
          ...
          let status: DatabaseSchemaMetrics['status'] = 'healthy';
          const errors: string[] = [];
          ...
          logger.warn(`Could not update metrics for schema ${schemaName}:`, schemaError);
          ...
          this.schemaMetrics.set(schemaName, {
            ...
            errors: [`Schema not accessible: ${(schemaError as Error).message}`],
          });
        }
      }

    } catch (error) {
      logger.error('Failed to update schema metrics:', error);
    }
  }

  private async getTableCountForSchema(schemaName: string): Promise<number> {
    try {
      ...
      logger.warn(`Could not get table count for schema ${schemaName}:`, error);
      return 0;
    }
  }

  async getSchemaMetrics(): Promise<DatabaseSchemaMetrics[]> {
  ...
  async getQueryMetrics(limit = 100): Promise<QueryMetrics[]> {
    return this.queryMetrics
      .sort((a, b) => b.timestamp.getTime() - a.timestamp.getTime())
      .slice(0, limit);
  }

  async getSlowQueries(threshold = 1000, limit = 50): Promise<QueryMetrics[]> {
    return this.queryMetrics
  ...
  async checkSchemaHealth(schemaName: string): Promise<DatabaseSchemaMetrics | null> {
...
// Singleton instance
let databaseMonitor: DatabaseMonitoringService | null = null;

export function getDatabaseMonitor(): DatabaseMonitoringService {
  if (!databaseMonitor) {
